...
it:{
  ...
  wA1P:"L'algoritmo utilizzato nelle simulazioni semplifica il volo utilizzando durante tutto lo spostamento il vento della colonna d'aria di partenza. Tuttavia, la sonda mentre vola si sposta anche di centinaia di chilometri, dove il vento è diverso. La differenza è in media circa il <strong>2 %</strong>. Trascurabile per quello che ci serve. Il calcolo corretto richiederebbe di utilizzare in ogni istante il valore del vento in quel punto; il confronto tra le due colonne d'aria mostra che questo non è necessario.",
  ...
  wA3P:"I dati mostrano che le ipotesi su cui si basa il modello gaussiano sono ragionevolmente soddisfatte. Tuttavia, va detto che i dati prodotti dal modello non sono tutti veramente indipendenti perché lo stesso sistema meteorologico governa più giorni consecutivi.",
...
es:{
  ...
  wA1P:"El algoritmo utilizado en las simulaciones simplifica el vuelo usando durante todo el desplazamiento el viento de la columna de aire de partida. Sin embargo, la sonda mientras vuela se desplaza también cientos de kilómetros, donde el viento es distinto. La diferencia es en promedio de alrededor del <strong>2 %</strong>. Despreciable para lo que necesitamos. El cálculo correcto exigiría usar en cada instante el valor del viento en ese punto; la comparación entre las dos columnas de aire muestra que no hace falta.",
  ...
  wA3P:"Los datos muestran que las hipótesis en las que se basa el modelo gaussiano se cumplen razonablemente. Hay que decir, sin embargo, que los datos producidos por el modelo no son todos verdaderamente independientes, porque el mismo sistema meteorológico gobierna varios días seguidos.",
...
en:{
  ...
  wA1P:"The algorithm used in the simulations simplifies the flight by using the wind of the starting air column throughout the whole displacement. While flying, however, the probe also moves hundreds of kilometres, where the wind is different. The difference averages about <strong>2 %</strong>. Negligible for our purposes. The correct calculation would require using the wind value at each point at every instant; comparing the two air columns shows that this is not necessary.",
  ...
  wA3P:"The data show that the assumptions behind the Gaussian model hold reasonably well. It should be said, however, that the data produced by the model are not all truly independent, because the same weather system governs several consecutive days.",
