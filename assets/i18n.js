...
it:{
  ...
  twHead:"La previsione del giorno",
  twP1:"Lo studio qui sopra dice dove va a finire la sonda <strong>in media</strong>. Nei giorni prima del lancio bisogna sapere dove andrà <strong>quel giorno</strong>. Per questo usiamo <strong>Tawhiri</strong>, il predittore di traiettorie per palloni sonda gestito da SondeHub: integra salita e discesa sui venti dell'ultima previsione del modello globale <strong>GFS</strong> della NOAA, che viene rifatta ogni sei ore.",
  twP2:"Tawhiri calcola solo dove porta il vento: la quota di scoppio e la velocità di discesa gliele diamo noi, dal nostro modello del pallone. La previsione copre circa <strong>una settimana</strong>, ed è tanto più affidabile quanto più il giorno è vicino. È una traiettoria sola, senza un'incertezza associata: per capire quanto fidarsi conviene ripetere il calcolo nei giorni successivi e guardare di quanto si sposta il punto di atterraggio.",
  twDate:"Giorno", twTime:"Ora di lancio (Uruguay)", twAsc:"Salita · m/s", twBurst:"Scoppio · km", twDesc:"Discesa al suolo · m/s",
  twRun:"Calcola le traiettorie", twWeek:"Confronta i prossimi giorni",
  ...
  twLegend:"Linea piena: salita · tratteggio: discesa · cerchio bianco: scoppio.",
  twWDay:"Giorno", twWeekNote:"Stessa ora e stessi parametri per tutti i giorni. Cliccando una riga, quella previsione compare sulla mappa.",
  twNote:"L'area di esclusione è il poligono approssimato usato anche nella simulazione. La previsione si calcola al momento, dal browser di chi guarda: non viene salvato niente.",
...
es:{
  ...
  twHead:"El pronóstico del día",
  twP1:"El estudio de arriba dice dónde termina la sonda <strong>en promedio</strong>. En los días previos al lanzamiento hay que saber adónde irá <strong>ese día</strong>. Para eso usamos <strong>Tawhiri</strong>, el predictor de trayectorias para globos sonda que mantiene SondeHub: integra el ascenso y el descenso sobre los vientos del último pronóstico del modelo global <strong>GFS</strong> de la NOAA, que se rehace cada seis horas.",
  twP2:"Tawhiri calcula solo adónde lleva el viento: la altura de reventado y la velocidad de descenso se las damos nosotros, a partir de nuestro modelo del globo. El pronóstico cubre alrededor de <strong>una semana</strong>, y es más confiable cuanto más cerca está el día. Es una sola trayectoria, sin una incertidumbre asociada: para saber cuánto fiarse conviene repetir el cálculo en los días siguientes y ver cuánto se mueve el punto de aterrizaje.",
  twDate:"Día", twTime:"Hora de lanzamiento (Uruguay)", twAsc:"Ascenso · m/s", twBurst:"Reventado · km", twDesc:"Descenso al suelo · m/s",
  twRun:"Calcular las trayectorias", twWeek:"Comparar los próximos días",
  ...
  twLegend:"Línea continua: ascenso · punteada: descenso · círculo blanco: reventado.",
  twWDay:"Día", twWeekNote:"Misma hora y mismos parámetros para todos los días. Al hacer clic en una fila, ese pronóstico aparece en el mapa.",
  twNote:"El área de exclusión es el polígono aproximado que se usó también en la simulación. El pronóstico se calcula en el momento, desde el navegador de quien lo mira: no se guarda nada.",
...
en:{
  ...
  twHead:"The forecast for the day",
  twP1:"The study above tells us where the probe ends up <strong>on average</strong>. In the days before the launch we need to know where it will go <strong>on that day</strong>. For this we use <strong>Tawhiri</strong>, the balloon trajectory predictor run by SondeHub: it integrates the ascent and the descent through the winds of the latest forecast of NOAA's global <strong>GFS</strong> model, which is rerun every six hours.",
  twP2:"Tawhiri only works out where the wind carries the balloon: the burst altitude and the descent rate come from us, from our own balloon model. The forecast covers about <strong>a week</strong>, and the closer the day, the more reliable it is. It is a single trajectory, with no uncertainty attached: to judge how far to trust it, repeat the calculation on the following days and see how much the landing point moves.",
  twDate:"Day", twTime:"Launch time (Uruguay)", twAsc:"Ascent · m/s", twBurst:"Burst · km", twDesc:"Descent at ground · m/s",
  twRun:"Compute the trajectories", twWeek:"Compare the next days",
  ...
  twLegend:"Solid line: ascent · dashed: descent · white circle: burst.",
  twWDay:"Day", twWeekNote:"Same time and same parameters for every day. Click a row to show that forecast on the map.",
  twNote:"The exclusion area is the approximate polygon also used in the simulation. The forecast is computed on the spot, in the viewer's browser: nothing is stored.",
