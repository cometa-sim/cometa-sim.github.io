...
it:{
  ...
  twP1:"Qui si prevede il volo di un giorno preciso. Si sceglie da dove e quando partire, si descrive il pallone, e la pagina calcola quanto elio serve, a che quota scoppierà e a che velocità scenderà. Poi chiede la traiettoria a <strong>Tawhiri</strong>, il predittore per palloni sonda gestito da SondeHub, che la integra sui venti dell'ultima previsione del modello globale <strong>GFS</strong> della NOAA, rifatta ogni sei ore.",
  ...
  twNote:"L'area di esclusione è il poligono approssimato usato anche nello studio. Il calcolo avviene nel browser di chi guarda e non viene salvato niente, tranne l'ultimo luogo scelto, che resta su questo dispositivo.",
  srcTw:"<strong>Previsione del giorno:</strong> predittore Tawhiri di <a href=\"https://sondehub.org/\" target=\"_blank\" rel=\"noopener\">SondeHub</a>, sui venti del modello GFS della <a href=\"https://www.noaa.gov/\" target=\"_blank\" rel=\"noopener\">NOAA</a>.",
  ...
  twIsa:"Calcolo con l'atmosfera standard (ISA): lo studio, con l'atmosfera vera di ogni giorno, trova quote di scoppio fra 37,5 e 38,2 km. Il paracadute ha coefficiente di resistenza 1,0 e massa 80 g, come quello del kit.",
  twManual:"Imposta a mano quota di scoppio e discesa",
  twManHint:"Se restano vuoti valgono i valori calcolati qui sopra.",
  twStart:"Partenza",
...
es:{
  ...
  twP1:"Acá se pronostica el vuelo de un día preciso. Se elige desde dónde y cuándo partir, se describe el globo, y la página calcula cuánto helio hace falta, a qué altura reventará y a qué velocidad bajará. Después le pide la trayectoria a <strong>Tawhiri</strong>, el predictor para globos sonda que mantiene SondeHub, que la integra sobre los vientos del último pronóstico del modelo global <strong>GFS</strong> de la NOAA, que se rehace cada seis horas.",
  ...
  twNote:"El área de exclusión es el polígono aproximado que se usó también en el estudio. El cálculo se hace en el navegador de quien lo mira y no se guarda nada, salvo el último lugar elegido, que queda en este dispositivo.",
  srcTw:"<strong>Pronóstico del día:</strong> predictor Tawhiri de <a href=\"https://sondehub.org/\" target=\"_blank\" rel=\"noopener\">SondeHub</a>, sobre los vientos del modelo GFS de la <a href=\"https://www.noaa.gov/\" target=\"_blank\" rel=\"noopener\">NOAA</a>.",
  ...
  twIsa:"Cálculo con la atmósfera estándar (ISA): el estudio, con la atmósfera real de cada día, encuentra alturas de reventado entre 37,5 y 38,2 km. El paracaídas tiene coeficiente de resistencia 1,0 y masa 80 g, como el del kit.",
  twManual:"Fijar a mano la altura de reventado y el descenso",
  twManHint:"Si quedan vacíos, valen los valores calculados arriba.",
  twStart:"Partida",
...
en:{
  ...
  twP1:"Here you forecast the flight on a given day. You choose where and when to launch and describe the balloon, and the page works out how much helium is needed, at what altitude the balloon will burst and how fast it will come down. It then asks <strong>Tawhiri</strong>, the balloon trajectory predictor run by SondeHub, for the trajectory, integrated through the winds of the latest forecast of NOAA's global <strong>GFS</strong> model, which is rerun every six hours.",
  ...
  twNote:"The exclusion area is the approximate polygon also used in the study. Everything is computed in the viewer's browser and nothing is stored, except the last launch site chosen, which stays on this device.",
  srcTw:"<strong>Forecast for the day:</strong> the Tawhiri predictor by <a href=\"https://sondehub.org/\" target=\"_blank\" rel=\"noopener\">SondeHub</a>, on the winds of the GFS model by <a href=\"https://www.noaa.gov/\" target=\"_blank\" rel=\"noopener\">NOAA</a>.",
  ...
  twIsa:"Computed with the standard atmosphere (ISA): the study, using the real atmosphere of each day, finds burst altitudes between 37.5 and 38.2 km. The parachute has a drag coefficient of 1.0 and a mass of 80 g, like the one in the kit.",
  twManual:"Set burst altitude and descent by hand",
  twManHint:"Left empty, the values computed above apply.",
  twStart:"Launch",
